...
export function onAceEditorAfterRenderEvent(aceEditor, codeEditor) {
...
var haveComplained = false;
function complainAboutHackOnce(message) {
  if (!haveComplained) {
...
function drawTextLayer(ctx, ed, scrollLeft) {
  ...
  const SETH_HACK_FIXME_TEXT_Y_OFFSET = 16;
  complainAboutHackOnce("HACKY FIXED TEXT OFFSET Y=16 IN THREE-CODEEDITOR/lib/rendering.js")
  ...
  var r            = ed.renderer,
      ...
      localCoords  = {
        x: config.padding,
        y: (config.firstRowScreen*config.lineHeight) - r.scrollTop + r.$fontSize + SETH_HACK_FIXME_TEXT_Y_OFFSET
      },
